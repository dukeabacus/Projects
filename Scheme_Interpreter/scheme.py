...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        args = target.second
        formals = target.first
        env.define(formals, LambdaProcedure(args, expressions.second, env) )
        return formals
        # END PROBLEM 10
    else:
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
       ...
       # print(cur.first)
        ...
        args = Pair(scheme_eval(cur.second.first,env),args)
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            # BEGIN Extra Credit
            result = scheme_apply(scheme_eval(first,env), rest.map(lambda operand: scheme_eval(operand,env)),env)
