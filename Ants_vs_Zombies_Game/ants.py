        ...
        if insect.is_ant:
            ...
            if self.ant is insect:
                ...
                if hasattr(self.ant, 'container') and self.ant.container and self.ant.ant is insect:
                    if self.ant.ant.name == 'Queen' and self.ant.ant.imposter == False:
                        return
                    self.ant.ant = None
                else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        place = self.place
        cur_range = 0
        while(place != None and place is not hive):
            bee = random_or_none(place.bees)
            if bee and cur_range >= self.min_range and cur_range <= self.max_range:
                return bee
            cur_range+=1
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def double_damage(ant):
        if ant is not None and not ant.doubled:
            ant.doubled = True
            ant.damage *=2            
    ...
    def action(self, colony):
        ...
        if self.imposter:
            ...
            while(location!=None):
                QueenAnt.double_damage(location.ant)
                if location.ant != None and location.ant.container:
                    QueenAnt.double_damage(location.ant.ant)
