...
public class GraphBuildingHandler extends DefaultHandler {
    ...
    public void startElement(String uri, String localName, String qName, Attributes attributes)
        ...
        if (qName.equals("node")) {
            ...
            g.addNode(attributes.getValue("id"),attributes.getValue("lon"), attributes.getValue("lat"));
            g.setCurNode(attributes.getValue("id"),
...
//            System.out.println("Tag with k=" + k + ", v=" + v + ".");
            ...
            g.getLocation().put(g.getCurNode().id,name);
        }
    }
    ...
    public void endElement(String uri, String localName, String qName) throws SAXException {
        if (qName.equals("way")) {
         ...
         //   System.out.println("Finishing a way...");
            if (g.getCurWay().isValid) {

                ArrayList<String> connections = g.getCurWay().vertices;
                if(connections.size() > 1) {
                    for (int i = 0; i < connections.size() - 1; i++) {
                        GNode A = g.getNode(connections.get(i));
                        GNode B = g.getNode(connections.get(i + 1));
                        A.connectNode( B.id );
                        B.connectNode( A.id );
                        g.addNode(A.id,A);
                        g.addNode(B.id,B);
